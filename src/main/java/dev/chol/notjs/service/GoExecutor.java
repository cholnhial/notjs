...
public class GoExecutor implements NotJSExecutor {
    ...
    public ExecutionHandle execute(String srcCode, String version, List<String> arguments) throws IOException {
        ...
        registerTempFile(goFilePath.toFile());
        ...
        Thread cleanupThread = new Thread(() -> {
            try {
                ...
                if (!handle.isCleanedUp()) {
                    log.info("Go process terminated naturally, cleaning up {} temporary files", filesToClean.size());
                    tempFiles.set(filesToClean);
                    cleanupTempFiles();
                    tempFiles.remove();
                } else {
                    log.debug("Go temp files already cleaned up via forceCleanup()");
                }
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                if (!handle.isCleanedUp()) {
                    log.warn("Go cleanup thread interrupted, performing cleanup anyway");
                    tempFiles.set(filesToClean);
                    cleanupTempFiles();
                    tempFiles.remove();
                }
